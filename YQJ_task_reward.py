...
import getJson,YQJ_task,YQJ_getInfo
...
def run(tk: str, vn: str):
    ...
    # 获取任务列表JSON
    task_list_json = YQJ_task.get_task_list(tk=tk,vn=vn)
    ...
    for task_json in task_list_json['result']:
        ...
        if task_json['status']!= "received":
            ...
            if task_json['status'] == "uncompleted":
                # 激活任务
                YQJ_task.post_task_complete(task_json['code'])

            # 遍历任务,如果任务未完成次数小于2：执行一次。否则else，遍历任务
            if (task_json['total'] - task_json['count'])>2:
                YQJ_task.post_reward(tk=tk, vn=vn, code=task_json['code'])
            else:

...
def default_headers(tk: str, vn: str):
    status_headers['tk'] = tk
    status_headers['vn'] = vn
    getBubble_headers['tk'] = tk
    getBubble_headers['vn'] = vn
    upgrade_headers['tk'] = tk
    upgrade_headers['vn'] = vn


# status_headers用于获取元气工厂状态的get请求头
status_headers = {
...
# getBubble_headers用于获取元气工厂元气的post请求头
getBubble_headers = {
    'pkg': 'com.yuanqijiang.beautify.collection.pets',
    'v': '1.2.9',
    'os': 'android',
...
# 获取元气工厂状态URL
url_getStatus = 'http://server.ipolaris-tech.com/factory/status'

# 获取元气工厂元气URL
url_getBubble = 'http://server.ipolaris-tech.com/factory/getBubble'

# 升级元气工厂URL
url_upgrade = 'http://server.ipolaris-tech.com/factory/upgrade'
...
def getBubble(tk: str, vn: str):
    ...
    if bubble_number >= 5:
        # 获取元气工厂元气
        Bubble_response = requests.post(url=url_getBubble, headers=getBubble_headers, data=getBubbleData)
        # 判断响应状态码
...
def getBubbleStatus():
    # 获取元气工厂状态
    status_response = requests.get(url=url_getStatus, headers=status_headers)
...
def upgrade():
    upgrade_response = requests.get(url=url_upgrade, headers=upgrade_headers)
    try:
